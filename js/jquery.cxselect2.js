...
(function (factory) {
  ...
  cxSelect.init = function (dom, settings) {
    ...
    self.close = cxSelect.close.bind(theSelect);
  ...
  cxSelect.setOptions = function (opts) {
    ...
    if (!$.isArray(self.selectArray) || !self.selectArray.length || (opts && opts.selects)) {
      ...
      if ($.isArray(self.settings.selects) && self.settings.selects.length) {
        ...
        for (var i = 0, l = self.settings.selects.length; i < l; i++) {
          _tempSelect = self.dom.box.find('.select-box.' + self.settings.selects[i]);
    ...
    for (var i = 0; i < self.selectArray.length; i++) {
      //展开收缩控制
      cxSelect.toggleOpts.call(self.selectArray[i], self.selectArray[i], self.selectArray);

      self.selectArray[i].on('click', function (e) {
        $(this).trigger('toggleOpts', e.target);
      });
  ...
  cxSelect.attach = function () {
    ...
    if (!self.attachStatus) {
      self.dom.box.on('change', '.select-box', self.changeEvent);
    };
  ...
  cxSelect.detach = function () {
    var self = this;
    self.dom.box.off('change', '.select-box', self.changeEvent);
    self.attachStatus = false;
  };

  cxSelect.close = function() {
    var self = this;
    $(document).click(function(e) {
      if ( $(e.target).parents('#custom_data').length === 0)
      {
        for(var i = 0; i < self.selectArray.length; i++) {
          self.selectArray[0].trigger('toggleOpts');
        }
  ...
  cxSelect.clear = function (index) {
    ...
    for (var i = index, l = self.selectArray.length; i < l; i++) {
      self.selectArray[i].find('.opts-wrap').remove();
      self.selectArray[i].find('.select-value').html();
  ...
  cxSelect.start = function (data) {
    ...
    for (var i = 0, l = self.selectArray.length; i < l; i++) {
      if (typeof self.selectArray[i].attr('data-value') !== 'string' && $(self.selectArray[i][0]).find('.opt').length) {
        self.selectArray[i].attr('data-value', $(self.selectArray[i][0]).find('.opt').eq(0).html());
      };
    };

    if (self.settings.data || (typeof self.selectArray[0].data('url') === 'string' && self.selectArray[0].data('url').length)) {
      cxSelect.getOptionData.call(self, 0);
    } else if ($(self.selectArray[0][0]).find('.opt').length && typeof self.selectArray[0].attr('data-value') === 'string' && self.selectArray[0].attr('data-value').length) {
      $(self.selectArray[0]).find('.select-value').html(self.selectArray[0].attr('data-value'));
    ...
    if (typeof _dataUrl === 'string' && _dataUrl.length) {
      if (index > 0) {
        for (var i = 0, j = 1; i < index; i++, j++) {
          ...
          _selectValue = $(self.selectArray[i]).find('.select-value').html();
      ...
      for (var i = 0; i < index; i++) {
        _valueIndex = cxSelect.getIndex($(self.selectArray[i][0]).data('selectedIndex'), typeof self.selectArray[i].data('required') === 'boolean' ? self.selectArray[i].data('required') : self.settings.required);
  ...
  cxSelect.buildOption = function (index, data) {
    ...
    _select.find('.select-value').html(_firstTitle)
    ...
    if (typeof _jsonName === 'string' && _jsonName.length) {
      ...
      for (var i = 0, l = data.length; i < l; i++) {
        _html += '<div class="opt" data-index="' + i + '" data-value="' + String(data[i][_jsonValue]) + '">' + String(data[i][_jsonName]) + '</div>';
      };

      // 数组即为值的数据
    } else {
      for (var i = 0, l = data.length; i < l; i++) {
        _html += '<div class="opt" data-index="' + i + '" data-value="' + String(data[i]) + '">' + String(data[i]) + '</div>';
      };
    };


    _select.append('<div class="opts-wrap" style="display: none">' + _html + '</div>');
    ...
    if (typeof _select.attr('data-value') === 'string') {
      _select.find('.select-value').html(String(_select.attr('data-value'))).removeAttr('data-value');

    } else if (self.settings.firstTitle !== '') {
      $(_select[0]).data('selectedIndex', -1);
      _select.find('.select-value').html(self.settings.firstTitle);
    } else {
      $(_select[0]).data('selectedIndex', 0);
      _select.find('.select-value').html(_select.find('.opt').eq(0).html());
    }

    if (_required || $(_select[0]).data('selectedIndex') > 0) {
      _select.trigger('change');
    };

    _select.click(function (e) {
      if ($(e.target).hasClass('opt')) {
        _select.data('selectedIndex', $(e.target).data('index'));
        _select.find('.select-value').html($(e.target).html());
        _select.trigger('change');
      }
    });
  ...
  cxSelect.toggleOpts = function (oSelect, selectArray) {
    var self = this;
    oSelect.on('toggleOpts', function (e, target) {
      var _oSelect = $(target).parents('.select-box');
      var _spreadStatus = _oSelect.data('spread');
      var _newStatus = !_spreadStatus;


      if (!target || $(target).hasClass('select-value')) {
        for (var i = 0; i < selectArray.length; i++) {
          selectArray[i].data('spread', false);
          selectArray[i].find('.opts-wrap').css({
            display: 'none'
          });
        }
      }

      if (target && ($(target).hasClass('select-value') || $(target).hasClass('opt'))) {
        _oSelect.data('spread', !_spreadStatus);
        if (_newStatus) {
          _oSelect.find('.opts-wrap').css({
            display: ''
          });
        } else {
          _oSelect.find('.opts-wrap').css({
